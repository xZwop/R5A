...
public class R5A implements EntryPoint {
	...
	public void onModuleLoad() {
		...
		// ArrayList<LogootIdContainer> idList=
		// LogootEngine.generateLineIdentier(p, q, 1, ID);
		...
		textArea.addKeyUpHandler(new KeyUpHandler() {
			...
			public void onKeyUp(KeyUpEvent event) {
				...
				System.out.println("=======Event : diff(\"" + old + "\",\""
						+ textArea.getText() + "\")===========");
				...
				for (Diff d : diff) {
					...
					if (d.operation == alma.logoot.shared.diff_match_patch.Operation.EQUAL) {
						...
						for (LogootIdContainer lic : idList) {
							patch.add(new OpInsert(lic, d.text.charAt(i)));
						...
						// Recuperation des LogootIdContainer
		 ...
		 * textArea.addKeyUpHandler(new KeyUpHandler() {
		 * 
		 * @Override public void onKeyUp(KeyUpEvent event) { // Création
		 * identifiantLogoot // Send int pos=textArea.getCursorPos();
		 * LogootIdContainer p=idTable.get(pos-1); LogootIdContainer
		 * q=idTable.get(pos); ArrayList<LogootIdContainer> idList=
		 * LogootEngine.generateLineIdentier(p, q, 1, ID);
		 * System.out.println(event.getNativeEvent().getString()); char
		 * content=(char) event.getNativeKeyCode();
		 * System.out.println("="+content+"="); Operation op=new
		 * OpInsert(idList.get(0), content); send(op);
		 * idTable.add(pos,idList.get(0)); log(pos,content,idList.get(0),p,q);
		 * //
		 * log.setText(pos+"-"+((char)event.getNativeKeyCode())+"-"+idList.get
		 * (0)+"entre (" // +p+"-"+q+")\n"+log.getText()); } });
		 */
	}
	...
	private void receive(Patch patch) {
		System.out.println("= receive patch =");
